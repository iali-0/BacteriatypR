...
tibble_prealloc_index <- function(n){
...
duck <- as_duckdb_tibble(df)

set.seed(123)
tble <- tibble_predefine(n)
set.seed(123)
...
get_tble_single <- function(){
  filter(tble,kmer == 20)
}

get_tble_three <- function(){
  filter(tble,kmer == 20|kmer == 30|kmer == 50)
}
...
get_tble_threeJ <- function(){
  filter_table <- tibble(kmer=c(20, 30, 50))
  inner_join(tble,filter_table, by = "kmer")
}
