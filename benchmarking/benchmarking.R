...
vector_magritter <- function(x){
  1:x %>% (\(i) i^2)()
}

list_magritter <- function(x){
  1:x %>% (\(i) i^2)()%>% as.list()
}
...
n <- 1e4
sourceCpp("benchmark.cpp")

microbenchmark(vector_grow_append(n),
               vector_grow_c(n),
               vector_grow_br(n),
...
vector_get_one_named <- function(x){
  index <- c("thf","wwk","adh","gxd","vih","jui","fdi","gse","yye","ppb")
  x["adh"]
}
...
vector_get_c <- function(x){
  index <- c(5, 10,15,20,25, 30,35,40,45,50)
  c(x[5],x[10],x[15],x[20],x[25],x[30],x[35],x[40],x[45],x[50])
}

vector_get_c_named <- function(x){
  index <- c("thf","wwk","adh","gxd","vih","jui","fdi","gse","yye","ppb")

  c(x["thf"],x["wwk"],x["adh"],x["gxd"],x["vih"],x["jui"],x["fdi"],
  x["gse"],x["yye"],x["ppb"])
}
...
long_vector <- (1:10e4)^2
long_list <- (1:10e4)^2 |> as.list()
short <- (1:100)^2
n <- expand.grid(letters, letters, letters)|>
  apply(X = _, 1, paste0, collapse = "")
#expand.grid(letters, letters, letters)|> both below give the same results
  #apply(1, paste0, collapse = "")
#apply(expand.grid(letters, letters, letters),1, paste0, collapse = "")
...
microbenchmark(vector_get_one(long_vector),
  ...
  separate_wider_delim(expr, delim = " ", names = c("f","data")) %>%
