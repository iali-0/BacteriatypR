...
set.seed(123)
sequence <- sample(c("A","T","G","C"), 1500, replace = TRUE)|>
  paste(collapse = "")
#calculate the lenghth of a string
#get_aal_kmers/get_kmer
...
#substr toupper and tolower

#seq_to_base4
sequenc_upper <- sequence
...
microbenchmark(b_toupper(sequenc_upper),
               b_toupper(sequence_lower),
               b_tolower(sequenc_upper),
               b_tolower(sequence_lower),


               i_toupper(sequenc_upper),
               i_toupper(sequence_lower),
               i_tolower(sequenc_upper),
               i_tolower(sequence_lower),

               r_toupper(sequenc_upper),
               r_toupper(sequence_lower),
               r_tolower(sequenc_upper),
...
microbenchmark(b_gsub(sequence_good),
               b_gsub(sequence_bad),
               i_replace(sequence_good),
               i_replace(sequence_bad),
               r_replace(sequence_good),
               r_replace(sequence_bad))

# need to convert ACGT to 0123
b_chartr <- function(){
