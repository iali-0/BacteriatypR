...
NumericMatrix calculate_log_probability(const NumericMatrix& kmer_genus_count,
                                        const NumericVector& calcul_word_specific_priors,
  ...
  //log(kmer_genus_count + calcul_word_specific_priors)/(genus_counts + 1)))
  for(int j = 0; j<n_genera;j++){
    int genus_counts_p1 = genus_counts[j] + 1;
    for(int i= 0; i< n_kmers;i++){
      log_probability(i,j) = log((kmer_genus_count(i,j) + calcul_word_specific_priors(i))/(genus_counts_p1));
